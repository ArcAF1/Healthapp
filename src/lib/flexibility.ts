...
export class RecoveryProtocol {
  getRecoveryRecommendations(indulgence: PlannedIndulgence): RecoveryPlan {
    ...
    switch (indulgence.type) {
      case 'alcohol':
        return { ...base, hydrationMultiplier: indulgence.intensity === 'heavy' ? 1.5 : 1.3, exerciseIntensityReduction: indulgence.intensity === 'heavy' ? 0.5 : 0.2, sleepTargetIncrease: 1, nutritionFocus: ['electrolytes', 'b-vitamins', 'antioxidants'] }
      case 'cheat-meal':
        return { ...base, hydrationMultiplier: 1.2, nutritionFocus: ['fiber', 'lean-protein', 'vegetables'] }
      case 'late-night':
        return { ...base, sleepTargetIncrease: 0.5, exerciseIntensityReduction: 0.3, nutritionFocus: ['complex-carbs', 'magnesium'] }
      default:
        return base
    }
  }
  ...
  getAlcoholRecommendations(gender: 'male' | 'female'): AlcoholGuideline {
    ...
    return { weeklyLimit, dailyLimit, alcoholFreeDays: 2, hydrationRatio: 1.5 }
  }
}
